...
RESULTS_ROOT = "./results/6.1-Source_Activation_Conj"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="A script to perform conjunction-deviant contrast of source activations.")
    ...
    if args.prepare:
        ...
        for subject in os.listdir(SUBJECTS_DIR):
            ...
            for counter_f, fname in enumerate(fnames):
                ...
                evoked_sta1 = epochs["block-2/sent-1", "block-3/sent-1"].average().crop(TMIN, TMAX)
                evoked_sta2 = epochs["block-2/sent-4", "block-3/sent-4"].average().crop(TMIN, TMAX)

                evoked_dev1 = epochs["block-2/sent-2", "block-3/sent-2"].average().crop(TMIN, TMAX)
                evoked_dev2 = epochs["block-2/sent-3", "block-3/sent-3"].average().crop(TMIN, TMAX)

                inv = read_inverse_operator(
                    os.path.join(inv_root, basename.replace("_meg-epo.fif", "-inv.fif"))
                )

                stc_sta1 = morph.apply(apply_inverse(evoked_sta1, inv, **INV_PARAMS))
                stc_sta2 = morph.apply(apply_inverse(evoked_sta2, inv, **INV_PARAMS))

                stc_dev1 = morph.apply(apply_inverse(evoked_dev1, inv, **INV_PARAMS))
                stc_dev2 = morph.apply(apply_inverse(evoked_dev2, inv, **INV_PARAMS))

                stc_diff1 = stc_dev2.data - stc_sta1.data
                stc_diff2 = stc_dev1.data - stc_sta2.data
                
                if stc_diff_sub is None:
                    stc_diff_sub = np.zeros(shape=(len(fnames), stc_diff1.shape[0], stc_diff1.shape[1]))
                    stc_sta_sub = np.zeros(shape=(len(fnames), stc_diff1.shape[0], stc_diff1.shape[1]))
                    stc_dev_sub = np.zeros(shape=(len(fnames), stc_diff1.shape[0], stc_diff1.shape[1]))
                
                stc_diff_sub[counter_f] = np.asarray([stc_diff1, stc_diff2]).mean(axis=0)
                stc_sta_sub[counter_f] = np.asarray([stc_sta1.data, stc_sta2.data]).mean(axis=0)
                stc_dev_sub[counter_f] = np.asarray([stc_dev1.data, stc_dev2.data]).mean(axis=0)
        ...
        del stc_sta, stc_dev, stc_data
        del stc_sta_sub, stc_dev_sub, stc_diff_sub
        del stc_diff1, stc_diff2, epochs, morph, inv
        del evoked_sta1, evoked_sta2, evoked_dev1, evoked_dev2
        gc.collect()
