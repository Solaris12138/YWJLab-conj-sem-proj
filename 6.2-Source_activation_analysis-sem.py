...
RESULTS_ROOT = "./results/6.2-Source_Activation_Sem"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="A script to perform semantic-deviant contrast of source activations.")
    ...
    if args.prepare:
        ...
        for subject in os.listdir(SUBJECTS_DIR):
            ...
            for counter_f, fname in enumerate(fnames):
                ...
                evoked_sta = epochs["block-1/sent-1", "block-1/sent-4"].average().crop(TMIN, TMAX)
                evoked_dev = epochs["block-1/sent-2", "block-1/sent-3"].average().crop(TMIN, TMAX)

                inv = read_inverse_operator(
                    os.path.join(inv_root, basename.replace("_meg-epo.fif", "-inv.fif"))
                )

                stc_sta_f = morph.apply(apply_inverse(evoked_sta, inv, **INV_PARAMS))
                stc_dev_f = morph.apply(apply_inverse(evoked_dev, inv, **INV_PARAMS))

                stc_diff_f = stc_dev_f.data - stc_sta_f.data
                ...
                stc_sta_sub[counter_f] = stc_sta_f.data
                stc_dev_sub[counter_f] = stc_dev_f.data
        ...
        del stc_sta_f, stc_dev_f, stc_diff_f, epochs, morph, inv
        del evoked_sta, evoked_dev
        gc.collect()
